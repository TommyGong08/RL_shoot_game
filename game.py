...
def draw_map(person, monster):
    ...
    # 表示九个格子
    rect = [0] * 110
    screen.fill(background_color)  # 填充颜色(设置为0，执不执行这行代码都一样)
    for i in range(10):
        for j in range(10):
            curr_rect = 10 * i + j
            if target == curr_rect:
                # 绿色表示终点
                rect[curr_rect] = pygame.draw.rect(screen, (0, 228, 0), ((j * step, i * step), (step, step)), width=0)
            elif curr_rect in wall:
                # 灰色表示墙
                rect[curr_rect] = pygame.draw.rect(screen, (192, 192, 192), ((j * step, i * step), (step, step)), width=0)
            elif curr_rect == person:
                # 红色表示人
                rect[curr_rect] = pygame.draw.rect(screen, (192, 0, 0), ((j * step, i * step), (step, step)), width=0)
            elif curr_rect in monster:
                # 画出怪兽
                rect[curr_rect] = pygame.draw.rect(screen, (138, 43, 226), ((j * step, i * step), (step, step)), width=0)
            else:
                # 其余填充白色
                rect[curr_rect] = pygame.draw.rect(screen, (255, 228, 181), ((j * step, i * step), (step, step)), width=1)
...
def JudgeWin():
    ...
    """
    if person == target and is_killed == True:
        return True
    else:
        return False
...
def reset():
    ...
    monster = [33, 37]
    print(person)
    return person, monster


def judge_collision(person):
    """判断人是否碰撞到墙或怪物"""
    if person in wall or person in monster or person < 0:
        return True
    else:
        return False
